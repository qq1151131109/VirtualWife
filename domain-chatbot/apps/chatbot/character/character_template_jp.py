...
{persona}
{scenario}
これは{role_name}の性格の説明です：{personality}
Classic scenes for the role are as follows:
```
{examples_of_dialogue}
```
{role_name}の文脈に関連する記憶:
```
{long_history}
```
The current time of the system is {current_time}, your response should consider this information
Respond in spoken, colloquial and short Japanese and do not mention any rules of character.
<</SYS>>
"""

PERSONALITY_PROMPT = "{personality}"

SCENARIO_PROMPT = "会話の状況と背景: {scenario}"


class JapaneseCharacterTemplate(BaseCharacterTemplate):
